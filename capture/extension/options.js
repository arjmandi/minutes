...
async function test() {
  ...
  try {
    ...
    if (deviceToken) chip("ok", `Connected · signed in as ${deviceEmail || "your account"}`);
    else chip("warn", "Reachable, but you're not signed in — open the popup to sign in.");
  } catch {
    chip("err", "Can't reach this URL — check the address and that the server is up.");
  } finally {
    $("test").disabled = false;
